...
export default function LibraryDetailClient({ library, initialSeatProducts, initialOrders, currentUser }: LibraryDetailClientProps) {
    ...
    useEffect(() => {
     ...
     const handleChat = useCallback(async () => {
        ...
        try {
            const { data: existingRoom } = await supabase.rpc('get_mutual_private_room', { p_user1_id: currentUser.id, p_user2_id: library.seller_id });
            ...
            const { data: newRoomId, error: newRoomError } = await supabase.rpc('create_chat_room_with_participants', { p_user1_id: currentUser.id, p_user2_id: library.seller_id });
            ...
            const { error: welcomeMessageError } = await supabase.from('chat_messages').insert({
                ...
                content: `Hi, I have a question about ${library.name}.`,
            });
